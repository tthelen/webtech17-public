...
class StaticApp(App):
    ...
    def index(self, request, response, pathmatch=None):
        resp = "<html><body><h1>Index of {}</h1><ul>".format(request.resource)
        for f in os.listdir(self.path):
            resp += "<li><a href='{fn}'>{fn}</a></li>".format(fn=request.resource+f)  # one file per line
        resp += "</ul></body>"
        response.send(body=resp)
