...
function submit_content(event) {
    ...
    var id=null;
    var content=document.querySelector("#file_content").value;  // textarea
    if (event.target.id == 'edit') {  // id der Form = Bearbeiten
        id = document.querySelector("#file_id").value;
    } else if (event.target.id == 'create') {  // id der Form = Neu anlegen
        id = document.querySelector("#file_name").value
    }
    if (id && id.match(/[0-9A-Za-z]+/)) {  // Dateiname in serverkompatibler Form
    ...
    } else {  // keine (gültige) ID/Name vorhanden: Sever soll neu vergeben.
...
var base_url='http://localhost:8080';  // Ziel
var theCredentials = null;  // Benutzername/Passwort
